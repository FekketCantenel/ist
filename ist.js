        ...
        if (Cookies.get('todoistToken') === undefined) {
            if (authCode && authState === 'todoist') {
                ...
                $.get('README.md', function (readme) {
                    ...
                    $('#task').append(
                        readmeHTML,
                        `<a href="https://todoist.com/oauth/authorize?client_id=${process.env.CLIENTID}&state=todoist&scope=data:read_write"><button>when ready, log in with Todoist</button></a>`
                    );
                });
                $('#spinner').hide();
                return false;
